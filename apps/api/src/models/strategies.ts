...
export const getStrategiesActiveFromDB = async (
  ...
  return strategies.filter((strategy: any) => {
    ...
    return {
      ...
      countPending: strategy.intentBatches.filter((intentBatch: any) => !intentBatch.executedAt && !intentBatch.cancelledAt).length,
      countCancelled: strategy.intentBatches.filter((intentBatch: any) => intentBatch.cancelledAt).length,
      countExecuted: strategy.intentBatches.filter((intentBatch: any) => intentBatch.executedAt).length,
    }
  })
};
