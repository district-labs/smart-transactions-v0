...
import { DBIntentBatchActiveItem } from "@district-labs/intentify-database";
...
async function generateHooksForIntentBatch(
  ...
  switch (intentBatch.strategyId) {
      ...
      return await generateHooksForLimitOrderBasic({
        ...
        inputToken: {
          ...
          decimals: await getTokenDecimals({
            // TODO: Use token list to get decimals
            chainId: intentBatch.chainId,
    ...
    default:
      throw new Error(
        `No hooks for intentBatch ${intentBatch.intentBatchHash}`,
      );
  }
}
