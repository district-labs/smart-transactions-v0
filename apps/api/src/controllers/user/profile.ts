...
export const userProfileIsSignedIn = async (
  ...
  try {
    ...
    return response.status(200).json({ address: session.address, isLoggedIn: !!session.address })
  } catch (error) {
    next(error);
  }
};
...
export const userProfileUpdate = async (
  ...
  try {
    ...
    if (existingUser) {
        await db.update(users).set(data).where(eq(users.address, session.address))
    } else {
