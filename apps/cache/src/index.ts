...
if (process.env.npm_lifecycle_event === "dev") {
  ponder.on(
    "IntentifySafeModuleLocal:IntentBatchExecuted",
    async ({ context, event }: { context: Context; event: any }) => {
      ...
      await dispatchIntentExecution(
        31337,
        event.params.intentBatchId,
        event.transaction,
      );
    },
  );
  ...
  ponder.on(
    "IntentifySafeModuleLocal:IntentBatchCancelled",
    async ({ context, event }: { context: Context; event: any }) => {
      const { IntentBatch } = context.entities;
      // Dispatch a message to the API to notify that the intent batch has been cancelled
      await dispatchIntentCancelled({
        intentBatchHash: event.params.root,
        transactionHash: event.transaction.hash,
        transactionTimestamp: event.block.timestamp,
      });
      await IntentBatch.upsert({
        id: event.params.intentBatchId,
        create: {
...
ponder.on(
  "IntentifySafeModuleGoerli:IntentBatchExecuted",
  async ({ context, event }: { context: Context; event: any }) => {
    const { IntentBatch } = context.entities;
    await IntentBatch.upsert({
      id: event.params.intentBatchId,
...
ponder.on(
  "IntentifySafeModuleGoerli:IntentBatchCancelled",
  async ({ context, event }: { context: Context; event: any }) => {
    const { IntentBatch } = context.entities;
    await IntentBatch.upsert({
      id: event.params.intentBatchId,
    ...
    await dispatchIntentCancelled({
      intentBatchHash: event.params.intentBatchId,
      transactionHash: event.transaction.hash,
      transactionTimestamp: event.block.timestamp,
    });
  },
