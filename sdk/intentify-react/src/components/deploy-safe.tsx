...
import { useGetSafeMultiCallAddress, useGetSafeMultiCallAddress } from "@/hooks/use-get-safe-multi-call-address";
...
export const DeploySafe = ({ children, className, salt = 1 }: DeploySafe) => {
  ...
  const deterministicWalletAddress = useWalletFactoryGetDeterministicWalletAddress({
  ...
  React.useEffect( () => {
    if(deterministicWalletAddress.data && intentifyModuleAddress) {
      console.log(deterministicWalletAddress.data, 'deterministicWalletAddress.data')
