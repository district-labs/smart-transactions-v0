    ...
    ChainlinkDataFeedHelper
{
    ...
    /// @param tokenInAmount The amount of tokens to be purchased.
    ...
    {
        data = abi.encode(tokenOut, tokenIn, tokenOutPriceFeed, tokenInPriceFeed, tokenInAmount, thresholdSeconds);
    }
        ...
        returns (bool)
    {
        ...
        uint256 tokenOutAmount = _calculateTokenOutAmount(tokenOut, tokenIn, tokenInAmount, derivedPrice);
        ...
        _unlock(intent, hook, tokenOutAmount, initialTokenInBalance);
    ...
    /// @notice Calculate the estimated amount of tokens to be sold. Based on the price of the tokens.
    /// @param tokenOut The token to be sold.
    /// @param tokenIn The token to be purchased.
    /// @param tokenInAmount The amount of tokens to be purchased.
    /// @param derivedPrice The derived price of the two tokens.
    /// @return tokenOutAmount The estimated amount of tokens to be sold.
    ...
    {
        tokenOutAmount = (tokenInAmount * uint256(derivedPrice) * 10 ** uint256(ERC20(tokenOut).decimals()))
            / (10 ** uint256(ERC20(tokenIn).decimals() + _CHAINLINK_DECIMALS));
    }
    ...
    /// @return tokenInAmount The amount of tokens to be purchased.
    ...
    /// @param hook Contains data related to hook.
    /// @param tokenOutAmount The estimated amount of tokens to be sold.
    ...
    function _unlock(
        ...
        returns (bool)
    {
        (address tokenOut, address tokenIn,,, uint256 tokenInAmount,) = _decodeIntent(intent);
