...
bytes32 constant INTENTBATCHEXECUTION_TYPEHASH = keccak256(
    "IntentBatchExecution(IntentBatch batch,Signature signature,Hook[] hooks)Hook(address target,bytes data)Intent(address root,address target,uint256 value,bytes data)IntentBatch(address root,bytes nonce,Intent[] intents)Signature(bytes32 r,bytes32 s,uint8 v)"
);
