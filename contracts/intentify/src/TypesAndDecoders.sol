...
bytes32 constant INTENT_TYPEHASH = keccak256("Intent(address root,address target,uint256 value,bytes data)");
...
bytes32 constant HOOK_TYPEHASH = keccak256("Hook(address target,bytes data,bytes instructions)");
...
bytes32 constant INTENTBATCHEXECUTION_TYPEHASH = keccak256(
    "IntentBatchExecution(IntentBatch batch,Signature signature,Hook[] hooks)Intent(address root,address target,uint256 value,bytes data)IntentBatch(address root,bytes nonce,Intent[] intents)Signature(bytes32 r,bytes32 s,uint8 v)Hook(address target,bytes data,bytes instructions)"
);
...
contract TypesAndDecoders {
    ...
    function GET_INTENT_PACKETHASH(Intent memory _input) public pure returns (bytes32) {
        bytes memory encoded =
            abi.encode(INTENT_TYPEHASH, _input.root, _input.target, _input.value, keccak256(_input.data));
    ...
    function GET_INTENTBATCH_PACKETHASH(IntentBatch memory _input) public pure returns (bytes32) {
        bytes memory encoded = abi.encode(
            INTENTBATCH_TYPEHASH, _input.root, keccak256(_input.nonce), GET_INTENT_ARRAY_PACKETHASH(_input.intents)
        );
    ...
    function GET_HOOK_PACKETHASH(Hook memory _input) public pure returns (bytes32) {
        bytes memory encoded = abi.encode(HOOK_TYPEHASH, _input.target, _input.data, _input.instructions);
