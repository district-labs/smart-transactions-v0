...
bytes32 constant INTENT_TYPEHASH = keccak256("Intent(IntentExecution exec,Signature signature)IntentExecution(address root,address target,bytes data)Signature(bytes32 r,bytes32 s,uint8 v)");
...
bytes32 constant INTENTBATCH_TYPEHASH = keccak256("IntentBatch(DimensionalNonce nonce,Intent[] intents)DimensionalNonce(uint128 queue,uint128 accumulator)Intent(IntentExecution exec,Signature signature)IntentExecution(address root,address target,bytes data)Signature(bytes32 r,bytes32 s,uint8 v)");
...
bytes32 constant INTENTBATCHEXECUTION_TYPEHASH = keccak256("IntentBatchExecution(IntentBatch batch,Signature signature,Hook[] hooks)DimensionalNonce(uint128 queue,uint128 accumulator)Hook(address target,bytes data)Intent(IntentExecution exec,Signature signature)IntentBatch(DimensionalNonce nonce,Intent[] intents)IntentExecution(address root,address target,bytes data)Signature(bytes32 r,bytes32 s,uint8 v)");
...
// BEGIN EIP712 AUTOGENERATED BODY
  ...
  function GET_INTENT_PACKETHASH (Intent memory _input) public pure returns (bytes32) {
    ...
    bytes memory encoded = abi.encode(
      INTENT_TYPEHASH,
      GET_INTENTEXECUTION_PACKETHASH(_input.exec),
  ...
  function GET_INTENT_ARRAY_PACKETHASH (Intent[] memory _input) public pure returns (bytes32) {
    ...
    for (uint i = 0; i < _input.length; i++) {
      encoded = bytes.concat(
        encoded,
        GET_INTENT_PACKETHASH(_input[i])
      );
    }
