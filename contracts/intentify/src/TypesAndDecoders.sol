...
bytes32 constant INTENTBATCH_TYPEHASH = keccak256("IntentBatch(bytes nonce,Intent[] intents)Intent(address root,address target,bytes data)");
...
bytes32 constant INTENTBATCHEXECUTION_TYPEHASH = keccak256("IntentBatchExecution(IntentBatch batch,Signature signature,Hook[] hooks)Hook(address target,bytes data)Intent(address root,address target,bytes data)IntentBatch(bytes nonce,Intent[] intents)Signature(bytes32 r,bytes32 s,uint8 v)");
