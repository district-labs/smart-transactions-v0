...
contract LimitOrderIntentTest is PRBTest, StdCheats {
    ...
    function test_RevertWhen_LimitOrderIntent_InsufficientTokenBalancePostHook() external {      
        ...
        hooks[1] = Hook({
            target: address(_swapRouter),
            // function swap(address account, address mevBot, address tokenOut, address tokenIn, uint256 amountOutMax, uint256 amountInMin) external returns (bool) {
            data: abi.encodeWithSignature("swap(address,address,address,uint256,uint256,address)", address(_intentify), address(_tokenA), address(_tokenB), startingBalance, endingBalance - 1, _tokenRouterReleaseIntent)
        });
