...
contract ERC20TipIntentUnitConcreteTest is BaseTest {
    ...
    address internal immutable _erc20TokenMock = address(0x4444);

    function setUp() public virtual {
        initializeBase();

        _erc20TipIntentHarness = new ERC20TipIntentHarness(address(_intentifySafeModuleMock));
    }
    ...
    function test_encodeIntent_Success() external {
        ...
        bytes memory expected = abi.encode(_erc20TokenMock, amount);

        bytes memory actual = _erc20TipIntentHarness.encodeIntent(_erc20TokenMock, amount);
    ...
    function test_exposed_decodeIntent_Success() external {
        ...
        Intent memory intent = Intent({
            ...
            data: _erc20TipIntentHarness.encodeIntent(_erc20TokenMock, amount)
        });

        (address decodedErc20Token, uint256 decodedAmount) = _erc20TipIntentHarness.exposed_decodeIntent(intent);

        assertEq(decodedErc20Token, _erc20TokenMock);
    ...
    function test_execute_RevertWhen_InvalidRoot() external {
        ...
        _erc20TipIntentHarness.execute(intent, hook);
    }
