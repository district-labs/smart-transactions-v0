...
contract IntentifySafeModuleTest is SafeTestingUtils {
    ...
    function test_SafeExecuteIntent_Success() external {
        ...
        IntentBatchExecution memory batchExecution = _craftIntentBundleExecution(address(_safeCreated), _intentifySafeModule);
        _intentifySafeModule.execute(address(_safeCreated), batchExecution);
    }

    function _craftIntentBundleExecution(address _safe, IntentifySafeModule _intentify) internal returns (IntentBatchExecution memory batchExecution) {
        ...
        batchExecution = IntentBatchExecution({
            batch: intentBatch,
            signature: Signature({
                r: r,
                s: s,
